...
public class ConfigBT extends AppCompatActivity {
    ...
    private class saveAllConfigAsyc extends AsyncTask<Void, Void, Void>  // UI thread
    {
        ...
        protected void onPreExecute() {
            ...
            if(Name.trim().equals("")) {
                cancelMsg = getString(R.string.module_name_cannot_be_empty);
                cancelled = true;
            }
            if(PIN.trim().equals("")) {
                cancelMsg = cancelMsg + getString(R.string.pin_cannot_be_empty);
                cancelled = true;
            }
            alert = builder.create();
            alert.show();
    ...
    private class connectRetrieveAsyc extends AsyncTask<Void, Void, Void>  // UI thread
    {
        ...
        protected void onPreExecute() {
            builder = new AlertDialog.Builder(ConfigBT.this);
            //
            builder.setMessage(getResources().getString(R.string.attempting_coonecteing_to_bt_msg))
